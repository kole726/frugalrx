/**
 * Test script to diagnose issues with drug API communication (CommonJS version)
 * 
 ...
 * 
 * Run with: node -r dotenv/config src/scripts/test-drug-api.cjs
 */
...
const TEST_DRUGS = [
  ...
  // Edge cases
  'Amoxicillin', // Capitalized
  'LISINOPRIL', // All caps
  'Ator-vastatin', // With hyphen
  'metformin hcl', // With space
  'simva_statin', // With underscore
  
  // Special characters
  'amoxicillin/clavulanate', // With slash
  'co-trimoxazole', // With hyphen
  'penicillin g', // With space
  'vitamin b12', // With number
  'hepatitis b', // With letter
  
  // Unusual formats
  'amoxil', // Brand name
  'lipitor', // Brand name
  'tylenol', // OTC brand name
  'advil', // OTC brand name
  'benadryl' // OTC brand name
];
...
async function searchDrugs(query, token) {
  try {
    ...
    console.log(`\nSearching for drugs with query: "${query}" (normalized: "${normalizedQuery}") at ${apiUrl}/drugs/names`);
