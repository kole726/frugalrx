...
export async function GET(
  ...
  try {
    ...
    console.log(`Direct Drugautocomplete API: Received query "${query}"`);
    
    if (!query) {
      console.error('Direct Drugautocomplete API: Missing search query');
    ...
    if (query.length < 2) {
      console.log('Direct Drugautocomplete API: Query too short:', query);
      return NextResponse.json(
    ...
    try {
      ...
      try {
        ...
        if (postResponse.ok) {
          ...
          if (Array.isArray(postData) && postData.length > 0) {
            ...
            console.log(`Direct Drugautocomplete API: Returning ${formattedResults.length} results from POST`);
        ...
        if (response.ok) {
          ...
          if (Array.isArray(apiData) && apiData.length > 0) {
            ...
            console.log(`Direct Drugautocomplete API: Returning ${formattedResults.length} results from GET`);
            return NextResponse.json(formattedResults, { headers: corsHeaders });
          }
        } else {
      ...
      if (process.env.NEXT_PUBLIC_FALLBACK_TO_MOCK === 'true') {
        console.log(`Direct Drugautocomplete API: API attempt failed, falling back to mock data for "${query}"`);
        ...
        console.log(`Direct Drugautocomplete API: Returning ${formattedResults.length} mock results for "${query}"`);
    ...
    console.log(`Direct Drugautocomplete API: No results found for "${query}"`);
    return NextResponse.json([], { headers: corsHeaders });
  } catch (error: any) {
