  ...
  try {
    ...
    try {
      ...
      if (searchResults && searchResults.length > 0) {
        const response = {
          brands: [{ name: `${drugName.toUpperCase()} (generic)`, isGeneric: true }] as { name: string, isGeneric: boolean }[],
          forms: ['TABLET', 'CAPSULE'] as string[],
          strengths: [] as string[],
          quantities: ['30 TABLET', '60 TABLET', '90 TABLET', 'custom'] as string[]
        };
    ...
    return {
      brands: [{ name: drugName.toUpperCase(), isGeneric: true }] as { name: string, isGeneric: boolean }[],
      forms: ['TABLET', 'CAPSULE'] as string[],
      strengths: ['50 mcg', '100 mcg', '25 mcg'] as string[],
      quantities: ['30 TABLET', '90 TABLET', '60 TABLET', 'custom'] as string[]
    };
  } catch (error) {
